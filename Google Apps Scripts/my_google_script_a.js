// ─── CONFIGURATION ──────────────────────────────────────────
const PARENT_FOLDER_ID = '1VoTODjs8XSLfF1H-bCYNa5PaScMUAudg';
const NOTIFY_EMAIL = 'sanyingxuan0711@gmail.com';
const WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbwa_cO97pv1j_vbYfEToDlFzM3RBTR4XQ20rOkVkdTw46_cL_5ANX_VRx9HoothWiTbug/exec';
const WP_API_KEY = '9f3c8a1d2b4e7f9c0a6d8e1b2c4f5a7d9e0c1b2a3d4e5f6a7b8c9d0e1f2a';

// Use a DIFFERENT secret for Script B
const SECOND_SCRIPT_WEB_APP_URL = 'https://script.google.com/macros/s/AKfycbxHs2zEQeFoAZshfhnoTB1cdNTTfenMqRn-s4ROQvfqX--ZD1dz-iz7oeim5OKU42f_eA/exec';
...
function doPost(e) {
  try {
    ...
    // Optional fan-out
    payload.second_shared_secret = SECOND_SHARED_SECRET;
...
function forwardPayload(payload) {
  try {
    UrlFetchApp.fetch(SECOND_SCRIPT_WEB_APP_URL, {
      ...
      headers: {
        'x-shared-secret': SECOND_SHARED_SECRET
      },
      payload: JSON.stringify(payload),
      muteHttpExceptions: true
    });
  } catch (err) {
    Logger.log('Forwarding failed: ' + err.toString());
...
function doGet() {
  return ContentService
    .createTextOutput(JSON.stringify({
      status: 'ok',
      service: 'Apps Script Project A',
      mode: 'webhook / push',
      time: new Date().toISOString(),
      message: 'Endpoint is live and waiting for POST payloads'
    }))
